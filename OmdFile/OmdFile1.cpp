...
// void COmdFile1::InitMsrValue()
// {
// 	m_MsrValue.clear();
// 	m_MsrValue.push_back(&m_MsrRGBWD);
// 	m_MsrValue.push_back(&m_Msr5Nits);
// 	m_MsrValue.push_back(&m_MsrW49);
// 	m_MsrValue.push_back(&m_MsrW9);
// 	m_MsrValue.push_back(&m_MsrD25);
// 	m_MsrValue.push_back(&m_MsrW5);
// }
...
// 	CString strTemp;
	...
	// 		strTemp.Format("%s", p_vciData->at(SubNum(clrtyp, PtNm, i)).GetSetupValue());
	// 		AfxMessageBox(strTemp);
			switch(isMV)
			{
				case Lv:    return (p_vciData->at(SubNum(clrtyp, PtNm, i)).GetLv()  > 0.0) ? TRUE : FALSE;
				case Sx:    return (p_vciData->at(SubNum(clrtyp, PtNm, i)).GetSx()  > 0.0) ? TRUE : FALSE;
				case Sy:    return (p_vciData->at(SubNum(clrtyp, PtNm, i)).GetSy()  > 0.0) ? TRUE : FALSE;
				case du:    return (p_vciData->at(SubNum(clrtyp, PtNm, i)).GetDu()  > 0.0) ? TRUE : FALSE;
				case dv:    return (p_vciData->at(SubNum(clrtyp, PtNm, i)).GetDv()  > 0.0) ? TRUE : FALSE;
				case T:     return (p_vciData->at(SubNum(clrtyp, PtNm, i)).GetT()   > 0  ) ? TRUE : FALSE;
				case duv:   return (p_vciData->at(SubNum(clrtyp, PtNm, i)).GetDuv() > 0.0) ? TRUE : FALSE;
				case X:     return (p_vciData->at(SubNum(clrtyp, PtNm, i)).GetX()   > 0.0) ? TRUE : FALSE;
				case Y:     return (p_vciData->at(SubNum(clrtyp, PtNm, i)).GetY()   > 0.0) ? TRUE : FALSE;
				case Z:     return (p_vciData->at(SubNum(clrtyp, PtNm, i)).GetZ()   > 0.0) ? TRUE : FALSE;
 			}
	return FALSE;
}
...
BOOL COmdFile1::IsMsr(ColorType clrtyp, PointNum PtNm)
{
	...
	BOOL re(FALSE);
	for (int i = 0; i<PtNm; ++i)
		if ((p_vciData->at(SubNum(clrtyp, PtNm, i)).GetLv()  > 0.0) &&
		    (p_vciData->at(SubNum(clrtyp, PtNm, i)).GetSx()  > 0.0) &&
		    (p_vciData->at(SubNum(clrtyp, PtNm, i)).GetSy()  > 0.0) &&
		    (p_vciData->at(SubNum(clrtyp, PtNm, i)).GetDu()  > 0.0) &&
		    (p_vciData->at(SubNum(clrtyp, PtNm, i)).GetDv()  > 0.0) &&
		    (p_vciData->at(SubNum(clrtyp, PtNm, i)).GetT()   > 0  ) &&
		    (p_vciData->at(SubNum(clrtyp, PtNm, i)).GetDuv() > 0.0) &&
		    (p_vciData->at(SubNum(clrtyp, PtNm, i)).GetX()   > 0.0) &&
		    (p_vciData->at(SubNum(clrtyp, PtNm, i)).GetY()   > 0.0) &&
		    (p_vciData->at(SubNum(clrtyp, PtNm, i)).GetZ()   > 0.0))
			re = FALSE;
		else
			re = TRUE;
	return re;
...
{
	for (int i = 0; i< ptN; ++i)
		tChain.push_back( p_vciData->at(SubNum(clrtye, ptN, i)) );
	return tChain;
}
