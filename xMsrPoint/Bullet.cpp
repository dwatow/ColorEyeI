...
BOOL Bullet::SetSx(float x)
{
    ...
    { 
        m_fSx = x; 
        m_strSx.Format(_T("%f"), m_fSx);  //輸出資料用
        return TRUE; 
    } 
    else 
        return FALSE; 
...
{ 
    ...
    {    
        m_fSy = y;
        m_strSy.Format(_T("%f"), m_fSy);  //輸出資料用
        return TRUE; 
    } 
    else
        return FALSE; 
...
{ 
    ...
    { 
        m_fLv = L;     
        m_strLv.Format(_T("%f"), m_fLv);  //輸出資料用
        return TRUE; 
    } 
    else    
        return FALSE; 
...
{ 
    m_fDuv = Duv;    
    m_strDuv.Format(_T("%f"), m_fDuv);  //輸出資料用
    return TRUE; 
}
...
{ 
    m_fDu  = Du;    
    m_strDu.Format(_T("%f"), m_fDu);  //輸出資料用
    return TRUE; 
}
BOOL Bullet::SetDv(float Dv)  
{ 
    m_fDv  = Dv;    
    m_strDv.Format(_T("%f"), m_fDv);  //輸出資料用
    return TRUE; 
}
...
{ 
    m_fX   = X;  
    m_strX.Format(_T("%f"), m_fX);  //輸出資料用
    return TRUE; 
}
BOOL Bullet::SetY(float Y)    
{ 
    m_fY   = Y;  
    m_strY.Format(_T("%f"), m_fY);  //輸出資料用
    return TRUE; 
}
BOOL Bullet::SetZ(float Z)    
{ 
    m_fZ   = Z;  
    m_strZ.Format(_T("%f"), m_fZ);  //輸出資料用
    return TRUE; 
}
////////////////////////////////////////
...
{
    return (m_strLv.IsEmpty() || m_strX.IsEmpty() || m_strSx.IsEmpty() || m_strT.IsEmpty() || m_strDu.IsEmpty() || \
            m_strY.IsEmpty() || m_strSy.IsEmpty() || m_strDuv.IsEmpty() || m_strDv.IsEmpty() || m_strZ) ? TRUE : FALSE ;
