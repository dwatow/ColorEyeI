...
: CDialog(CCaSetupDlg::IDD, pParent)
{
    BeginWaitCursor();
...
BOOL CCaSetupDlg::OnInitDialog() 
{
    ...
    int i(0);
    for (i = 0; i <=   4; ++i) m_cbxSynMode.AddString(m_pCA210->GetSynMode((SynMode)i));
    for (i = 0; i <=   8; ++i) m_cbxDisMode.AddString(m_pCA210->GetDisplayMode((DisPlay)i));
    for (i = 0; i <=   1; ++i) m_cbxDisDigMode.AddString(m_pCA210->GetDisplayDigits((DisDigits)i));
    for (i = 0; i <=   2; ++i) m_cbxAvgMode.AddString(m_pCA210->GetAvgingMode((AvgMode)i));
    for (i = 0; i <=   1; ++i) m_cbxBrigUnit.AddString(m_pCA210->GetBrigUnit((BrigUnit)i));
    for (i = 1; i <= 1+1; ++i) m_cbxDisCalStand.AddString(m_pCA210->GetCalStandard((CalStand)i));
    for (i = 0; i <  100; ++i)
    {
        m_pCA210->SetChNo((long)i);
